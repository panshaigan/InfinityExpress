...
fn fallback_snapshot_name() -> String {
    ...
    let compact = stamp
        ...
        .collect::<String>();
    if compact.len() >= 14 {
        format!("snapshot-{}-{}", &compact[0..8], &compact[8..14])
    } else {
...
pub async fn backup_game_dir(
    ...
    if dest.exists() {
        remove_path_with_progress(&app, dest.clone(), "delete", "Removing previous backup…")
            .await?;
    }
...
pub async fn restore_game_dir(
    ...
    wipe_dir_with_progress(&app, target.clone(), "delete", "Cleaning game folder…").await?;
...
pub async fn delete_backup(
    ...
    let matches = |entry_path: &str| {
        Path::new(entry_path) == path
            || fs::canonicalize(entry_path).ok().is_some_and(|p| p == safe)
    };
    ...
    emit_progress(&app, progress("done", "Backup deleted", 0, 0, 0, 0));
    Ok(())
}
...
pub async fn prepare_project_destination(
    ...
    if empty {
        ...
        let Some(vanilla) = vanilla else {
            return Err(
                "Destination folder is empty — set a vanilla backup first so it can be copied"
                    .into(),
            );
        };
        if !vanilla.is_dir() {
