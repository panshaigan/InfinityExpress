...
async fn probe_weasel(url: &str) -> Result<RemoteProbeResult, String> {
  ...
  let ver_re =
    Regex::new(r"(?s)<strong>Version</strong><br\s*/?>\s*([^<]+)").map_err(|e| e.to_string())?;
...
async fn probe_israelbarbuzano(url: &str) -> Result<RemoteProbeResult, String> {
  ...
  let dl_re = Regex::new(r#"href="(http://www\.israelbarbuzano\.net/imoen/files/[^"]+)""#)
    .map_err(|e| e.to_string())?;
  let ver_re =
    Regex::new(r"(?i)DOWNLOAD</a>\s*-\s*v([\d.]+)").map_err(|e| e.to_string())?;
...
async fn probe_baldurs_gate_de(url: &str) -> Result<RemoteProbeResult, String> {
  ...
  .map_err(|e| e.to_string())?;
  let date_re = Regex::new(
    r#"(?s)<dt>Letzte Bearbeitung</dt>\s*<dd><time[^>]*data-date-string="([^"]+)""#,
  )
  .map_err(|e| e.to_string())?;
  let version = ver_re
    .captures(&html)
...
fn extract_with_7z(archive: &Path, dest: &Path) -> Result<(), String> {
...
fn extract_archive(archive: &Path, dest: &Path, ext: &str) -> Result<(), String> {
  fs::create_dir_all(dest).map_err(|e| e.to_string())?;
  match ext {
    "zip" => extract_zip(archive, dest),
    "7z" => extract_7z(archive, dest).or_else(|_| extract_with_7z(archive, dest)),
    "rar" => extract_with_7z(archive, dest),
