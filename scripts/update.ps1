...
function DownloadWeb {
    ...
    Invoke-WebRequest -Uri $downloadLink -OutFile $downloadedFile -TimeoutSec 7200

    if ($extension -eq "zip") {
        Expand-Archive -Path $downloadedFile -DestinationPath $tempDir
    } elseif ($extension -eq "rar" -or $extension -eq "7z") {
...
Import-Csv -Path $script:csvFile | ForEach-Object {
    ...
    if ($URL -match "github\.com" -or $URL -match "baldurs-gate\.de" -or $URL -match "downloads\.weaselmods\.net" -or $URL -match "morpheus-mart\.com" -or $URL -match "israelbarbuzano\.net")
    {
        try
        {
            $modPath = Join-Path -Path $script:modsFolder -ChildPath $Codename
            ...
            if ($URL -match "israelbarbuzano\.net")
            {
                ...
                $downloadLink  = [regex]::Match($html, 'href="(http://www\.israelbarbuzano\.net/imoen/files/[^"]+)"').Groups[1].Value
            ...
            if ($URL -match "github\.com")
            {
                ...
                } else
                {
                    try
                    {
                        ...
                        if ($useAssetsFlag) {
                            $archives = @($response.assets | Where-Object {
                                $_.name -match '\.(zip|7z|rar)$'
                            })
                            if ($archives.Count -gt 0) {
                                $asset = $archives | Sort-Object -Property size -Descending | Select-Object -First 1
                                $downloadLink = $asset.browser_download_url
                                $extension = [System.IO.Path]::GetExtension($asset.name).TrimStart('.').ToLowerInvariant()
                            }
                            else {
