...
| **Filters** | Always | Search (component XML `label`, WeiDU `name`, component id) plus Status / Category / Mod selects, styled like the Mods facets. Column headers sort the visible rows (`#` default). Not persisted. `F3` / `Ctrl+F` / `/` focus the search field. |
