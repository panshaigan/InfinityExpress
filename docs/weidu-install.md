...
| **Previous** | `idle` / `paused` / `stopped` / `failed` (or before first Play); `cursor > 0` | Confirms, then moves cursor back one install step. Force-uninstalls that package via `setup-{weiduId}.exe` when it may be on disk; `queued` / `skipped` are status-reset only (no WeiDU uninstall). Creates an idle run via `ensureIdleRun()` when used before Play. |
...
| **Hide installed** | Always | Toolbar icon: hides rows with status `succeeded` / `alreadyInstalled` from the plan table. Persisted in the install session. Combined with the facet filters below (AND). |
| **Filters** | Always | Display-only. Search (component XML `label`, WeiDU `name`, component id) plus a Status select, styled like the Mods facets. Column headers sort **visible rows only** (`#` default). Play / Skip / Previous / cursor still walk the plan in install order. Not persisted. `F3` / `Ctrl+F` / `/` focus the search field. |
...
Install table rows (one per component / install step) expose row actions via a right-click menu and an **Actions** column — visible before the first **Play** as well (disabled/enabled per rules below). Breakpoints can be set on an idle plan via `ensureIdleRun()`. Toolbar **Previous** / **Skip** are not duplicated in the menu.
...
| **Uninstall back to here** | `idle` / `paused` / `stopped` / `failed`; target step before cursor | Force-uninstall each package from cursor−1 down to the target step (Done / Installed / failed, etc.); `queued` / `skipped` are status-reset only (no WeiDU uninstall). Move cursor to target. Confirms first. Blocked while `running` / `waitingForInput`. |
...
On Windows, WeiDU and 7-Zip are spawned with `CREATE_NO_WINDOW` so no console window appears; stdout/stderr/stdin stay in the app console. Game exe FileVersion is read in-process via Win32 (no PowerShell).
...
| UI | `src/ui/install/InstallStation.tsx`, `InstallTable.tsx`, `InstallFinishedDialog.tsx`, `RestoreSnapshotDialog.tsx`, `PlanSnapshotDialog.tsx`, `RestartConfirmDialog.tsx`, console dock |
