...
- **Done** — marks the current station finished (✓ in header and rail), then advances to the next unfinished non-empty screen (same cycle as Next, including wrap). Finished stations are skipped by Previous / Next / Done.
- **Reopen** — when the current station is already finished, the Done control becomes Reopen and clears the finished mark (station re-enters the Previous / Next cycle).
- **Previous** / **Next** — wrap through non-empty component screens after the active filters, skipping stations already marked finished. Engine is not a cycle stop (from Engine, Next enters the first unfinished screen and Previous the last). Non-content stations are one screen when their filtered list has rows. Content expands as main branch → each subbranch with filtered children (canonical sub order); Done on Content finishes the whole station and skips its remaining tabs.
